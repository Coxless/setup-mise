...
```yaml
- uses: Coxless/setup-mise@v1.0.0
```
...
Organization のポリシーによっては、GitHub Marketplace の verified action しか実行できない場合があります。このアクションは `action.yml` と `run.sh` の 2 ファイルで完結するシンプルな構成なので、自リポジトリにコピーして使うことができます。コードが小さく読みやすいため、動作を把握したうえで取り込めるのが利点です。

### ファイルをコピーする

`action.yml` と `run.sh` を自リポジトリの任意のディレクトリに配置します。
